...
function Ensure-Sam2Source {
  ...
  if (-not (Test-ZipArchive $Sam2Archive $Sam2ArchiveRootName)) {
    ...
    for ($attempt = 1; $attempt -le 8; $attempt++) {
      ...
      $curlArgs = @('--fail','--location','--http1.1','--ssl-revoke-best-effort','--continue-at','-','--retry','20','--retry-all-errors','--retry-delay','2','--connect-timeout','30','--speed-limit','1024','--speed-time','120','--output',$Sam2Part,$Sam2ArchiveUrl)
...
function Install-And-VerifySam2 {
  ...
  $env:PIP_RETRIES = '20'
  $env:PIP_TIMEOUT = '180'
  $installExit = Start-NativeVisible 'V4_INSTALL_SAM2_FROM_VERIFIED_LOCAL_SOURCE' $UvExe @('pip','install','--python',$PythonExe,'--link-mode','copy',$Sam2SourceRoot)
...
function Ensure-RuntimeMarker {
  $expected = [ordered]@{schema_version='1.0';python='3.10';torch='2.9.1';torchvision='0.24.1';cuda_wheel='cu128';gsplat='1.5.3';vggt_code_commit='a288dd0f14786c93483e45524328726ab7b1b4ce';vggt_model='facebook/VGGT-1B-Commercial';sam2_commit=$Sam2Commit;sam2_model='facebook/sam2.1-hiera-base-plus';recovery='SAM2_OFFICIAL_CODELOAD_EXACT_COMMIT'}
...
function Ensure-Recon3dLocalCache {
  ...
  if (-not $hasCommit) {
    ...
    for ($attempt = 1; $attempt -le 12; $attempt++) {
      ...
      $fetchExit = Start-NativeVisible 'V4_FETCH_RECON3D_PINNED' 'git' @('-c','http.version=HTTP/1.1','-c','http.maxRequests=1','-c','http.lowSpeedLimit=1','-c','http.lowSpeedTime=600','-C',$Recon3dCache,'fetch','--no-tags','--depth=1','origin',$Recon3dCommit)
...
try {
  ...
  $resumeArgumentLine = @('-NoProfile','-ExecutionPolicy','Bypass','-File',(Quote-WindowsCommandLineArg $RecoveryPath),'-RepoRoot',(Quote-WindowsCommandLineArg $RepoRoot),'-Branch',(Quote-WindowsCommandLineArg $Branch),'-ExpectedHead',(Quote-WindowsCommandLineArg $ExpectedHead),'-SiteId','"drift-curio"','-Sku','"DC-ZY-SZ-31001"','-VideoPath',(Quote-WindowsCommandLineArg $VideoPath)) -join ' '
