...
try {
  ...
  Write-Host "==> Sync exact audited P5 Kontext D0 head" -ForegroundColor Cyan
  ...
  $safety = "safety/local-before-p5-kontext-d0-$stamp"
  ...
  Write-Host "P5_QA01_V2_KONTEXT_D0_LOCAL_PREP=PASS" -ForegroundColor Green
  Write-Host ("HEAD=" + $head)
  Write-Host "==> Launch one adaptive Aquarium Kontext evaluation candidate" -ForegroundColor Cyan
  ...
  $script = Join-Path $RepoRoot "tools\p5_qa01_kontext_d0_eval.py"
  if (-not (Test-Path -LiteralPath $script -PathType Leaf)) { throw "KONTEXT_D0_SCRIPT_NOT_FOUND" }
  ...
  if ($code -ne 0) { throw "P5_KONTEXT_D0_EVAL_FAILED: exit=$code" }

  if (Get-Command Set-Clipboard -ErrorAction SilentlyContinue) {
    try { Set-Clipboard -Value (($output | ForEach-Object { [string]$_ }) -join [Environment]::NewLine) } catch {}
  }
  Write-Host "P5_QA01_V2_KONTEXT_D0_LOCAL_GATE=PASS" -ForegroundColor Green
  exit 0
} catch {
  Write-Host "P5_QA01_V2_KONTEXT_D0_LOCAL_GATE=FAIL" -ForegroundColor Red
