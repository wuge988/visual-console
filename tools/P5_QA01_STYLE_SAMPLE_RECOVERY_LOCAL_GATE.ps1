...
try {
  ...
  Write-Host "==> Sync exact audited P5 style-recovery head" -ForegroundColor Cyan
  ...
  if ($current -ne $remote) {
    $safety = "safety/local-before-p5-style-recovery-" + (Get-Date -Format "yyyyMMdd-HHmmss")
  ...
  if ($finalStatus.Count -gt 0) { throw ("WORKTREE_NOT_CLEAN_BEFORE_STYLE_RECOVERY :: " + ($finalStatus -join " | ")) }

  Write-Host "P5_QA01_STYLE_SAMPLE_RECOVERY_LOCAL_PREP=PASS" -ForegroundColor Green
  Write-Host ("HEAD=" + $head)
  Write-Host "==> Recover existing Aquarium backgrounds without SDXL rerun" -ForegroundColor Cyan

  & powershell.exe -NoProfile -ExecutionPolicy Bypass -File (Join-Path $RepoRoot "tools\P5_QA01_STYLE_SAMPLE_RECOVERY_GATE.ps1") -Sku $Sku -ExpectedHead $remote
  $code = $LASTEXITCODE
  if ($code -ne 0) { throw ("P5_QA01_STYLE_SAMPLE_RECOVERY_GATE_FAILED: exit=" + $code) }

  Write-Host "P5_QA01_STYLE_SAMPLE_RECOVERY_LOCAL_GATE=PASS" -ForegroundColor Green
  exit 0
} catch {
  $summary = @(
    "P5_QA01_STYLE_SAMPLE_RECOVERY_LOCAL_GATE=FAIL",
