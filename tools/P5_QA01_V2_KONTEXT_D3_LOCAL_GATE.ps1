...
try {
  ...
  Write-Host "==> Sync exact audited P5 Kontext D3 head" -ForegroundColor Cyan
  ...
  $safety = "safety/local-before-p5-kontext-d3-" + (Get-Date -Format "yyyyMMdd-HHmmss")
  ...
  Write-Host "P5_QA01_V2_KONTEXT_D3_LOCAL_PREP=PASS" -ForegroundColor Green
  Write-Host ("HEAD=" + $head)
  Write-Host "==> Launch two-stage masked Aquarium candidate" -ForegroundColor Cyan

  $python = "D:\AI\APPS\ComfyUI_windows_portable\python_embeded\python.exe"
  $script = Join-Path $RepoRoot "tools\p5_qa01_kontext_d3_eval.py"
  if (-not (Test-Path -LiteralPath $python -PathType Leaf)) { throw "COMFYUI_EMBEDDED_PYTHON_NOT_FOUND" }
  if (-not (Test-Path -LiteralPath $script -PathType Leaf)) { throw "KONTEXT_D3_SCRIPT_NOT_FOUND" }

  $toolsDir = Join-Path $RepoRoot "tools"
  $bootstrap = Join-Path $env:TEMP ("P5_QA01_D3_BOOTSTRAP_" + $ExpectedHead.Trim().Substring(0,12) + ".py")
  $launcher = @'
import runpy
import sys
...
sys.argv[0] = script
sys.stdout.write("P5_D3_PYTHON_BOOTSTRAP=TOOLS_DIR_INSERTED\n")
sys.stdout.flush()
...
'@
  ...
  if ($code -ne 0) { throw "P5_KONTEXT_D3_EVAL_FAILED: exit=$code" }

  $reviewLine = @($output | % { [string]$_ } | ? { $_ -like "review_file=*" } | Select-Object -Last 1)
  if ($reviewLine.Count -gt 0) {
    $review = ([string]$reviewLine[0]).Substring("review_file=".Length)
    if (Test-Path -LiteralPath $review -PathType Leaf) {
      Start-Process $review | Out-Null
    }
  }
  ...
  Write-Host "P5_QA01_V2_KONTEXT_D3_LOCAL_GATE=PASS" -ForegroundColor Green
  exit 0
} catch {
  Write-Host "P5_QA01_V2_KONTEXT_D3_LOCAL_GATE=FAIL" -ForegroundColor Red
