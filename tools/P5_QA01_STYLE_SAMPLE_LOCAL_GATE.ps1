...
try {
  ...
  Write-Host "==> Sync exact audited P5 style-sample head" -ForegroundColor Cyan
  ...
  if ($current -ne $remote) {
    $safety = "safety/local-before-p5-style-sample-" + (Get-Date -Format "yyyyMMdd-HHmmss")
  ...
  if ($finalStatus.Count -gt 0) { throw ("WORKTREE_NOT_CLEAN_BEFORE_STYLE_SAMPLE :: " + ($finalStatus -join " | ")) }

  Write-Host "P5_QA01_STYLE_SAMPLE_LOCAL_PREP=PASS" -ForegroundColor Green
  Write-Host ("HEAD=" + $head)
  Write-Host "==> Generate isolated Aquarium candidates" -ForegroundColor Cyan

  & powershell.exe -NoProfile -ExecutionPolicy Bypass -File (Join-Path $RepoRoot "tools\P5_QA01_STYLE_SAMPLE_GATE.ps1") -Sku $Sku -ExpectedHead $remote
  $code = $LASTEXITCODE
  if ($code -ne 0) { throw ("P5_QA01_STYLE_SAMPLE_GATE_FAILED: exit=" + $code) }

  Write-Host "P5_QA01_STYLE_SAMPLE_LOCAL_GATE=PASS" -ForegroundColor Green
  exit 0
} catch {
  $summary = @(
    "P5_QA01_STYLE_SAMPLE_LOCAL_GATE=FAIL",
