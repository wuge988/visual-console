...
function Get-ApiJson {
  param([string]$Path)
  return Invoke-RestMethod -Uri ($ApiBase + $Path) -Method Get -TimeoutSec 15
}

function Post-ApiJson {
  param([string]$Path, [object]$Body = $null)
  if ($null -eq $Body) {
    return Invoke-RestMethod -Uri ($ApiBase + $Path) -Method Post -TimeoutSec 20
  }
...
try {
  ...
  if (-not (Test-Path -LiteralPath $profilePath -PathType Leaf)) {
    throw "SITE_PROFILE_NOT_FOUND: $profilePath"
  }
  ...
  $archiveRows = @(Get-ApiJson "/api/archive?site_id=$siteEsc&item_id=$skuEsc")
  $sw01Archives = @($archiveRows | Where-Object {
    $_.workflow_code -eq "SW01" -and
    $_.destination_key -eq "white" -and
    $_.result -eq "VERIFIED_ARCHIVE"
  } | Sort-Object archived_at -Descending)
  if ($sw01Archives.Count -lt 1) {
    throw "NO_SW01_VERIFIED_ARCHIVE_FOR_SKU"
  }
  ...
  $derivatives = @(Get-ApiJson "/api/derivatives?site_id=$siteEsc&item_id=$skuEsc")
  $derivative = @($derivatives | Where-Object { $_.generated_asset_id -eq $assetId }) | Select-Object -First 1
  if ($null -eq $derivative) {
    throw "DERIVATIVE_RECORD_NOT_FOUND_FOR_ARCHIVE"
  }
  ...
  if ($sw01History.Count -ne 1) {
    throw "SW01_MANIFEST_HISTORY_NOT_EXACTLY_ONE"
  }
  $sw01 = $sw01History[0]
  ...
  $sourceHistory = @($history | Where-Object {
    $_.asset_id -eq $sourceAssetId -and
    $_.gate -eq "15" -and
    $_.workflow_code -eq "SC01" -and
    $_.destination_key -eq "cutout" -and
    $_.result -eq "VERIFIED_ARCHIVE"
  })
  Add-Check "SOURCE_MANIFEST_HISTORY" ($sourceHistory.Count -eq 1) "matching_rows=$($sourceHistory.Count)"
  if ($sourceHistory.Count -ne 1) {
    throw "SOURCE_MANIFEST_HISTORY_NOT_EXACTLY_ONE"
  }
  ...
  if (-not (Test-Path -LiteralPath $derivativeJournal -PathType Leaf)) {
    throw "DERIVATIVE_JOURNAL_NOT_FOUND"
  }
  $derivativeSnapshots = @()
  foreach ($line in [IO.File]::ReadAllLines($derivativeJournal, [Text.Encoding]::UTF8)) {
    ...
    try {
      ...
      if ($row.event -eq "DERIVATIVE_SNAPSHOT" -and $row.generated_asset_id -eq $assetId) {
        $derivativeSnapshots += $row
      }
    } catch { }
  }
  ...
  if (Test-Path -LiteralPath $archiveJournal -PathType Leaf) {
    foreach ($line in [IO.File]::ReadAllLines($archiveJournal, [Text.Encoding]::UTF8)) {
      ...
      try {
        $row = $line | ConvertFrom-Json
        if ($row.event -eq "ARCHIVE_SNAPSHOT" -and $row.asset_id -eq $assetId) {
          $archiveSnapshots += $row
        }
      } catch { }
    }
  }
  ...
  $archiveSnapshotOk = (
    $null -ne $latestArchive -and
    $latestArchive.workflow_code -eq "SW01" -and
    $latestArchive.destination_key -eq "white" -and
    $latestArchive.source_deleted -eq $true -and
    [string]$latestArchive.sha256 -eq [string]$sw01.sha256 -and
