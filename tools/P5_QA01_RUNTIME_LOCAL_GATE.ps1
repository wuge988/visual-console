...
try {
  ...
  Write-Host "==> Sync exact audited P5 runtime head" -ForegroundColor Cyan
  ...
  if ($current -ne $remote) {
    $safety = "safety/local-before-p5-runtime-" + (Get-Date -Format "yyyyMMdd-HHmmss")
  ...
  if ($finalStatus.Count -gt 0) { throw ("WORKTREE_NOT_CLEAN_BEFORE_RUNTIME_GATE :: " + ($finalStatus -join " | ")) }

  Write-Host "P5_QA01_RUNTIME_LOCAL_PREP=PASS" -ForegroundColor Green
  Write-Host ("HEAD=" + $head)
  Write-Host "==> Launch targeted ComfyUI runtime Gate" -ForegroundColor Cyan
  ...
  if ($runtimeCode -ne 0) { throw ("P5_QA01_RUNTIME_GATE_FAILED: exit=" + $runtimeCode) }

  Write-Host "P5_QA01_RUNTIME_LOCAL_GATE=PASS" -ForegroundColor Green
  exit 0
} catch {
  $summary = @(
    "P5_QA01_RUNTIME_LOCAL_GATE=FAIL",
