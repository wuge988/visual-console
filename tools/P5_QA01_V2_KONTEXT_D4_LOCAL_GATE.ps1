param(
  ...
  [string]$PriorEvidenceDir = "E:\AI_PROJECTS\DRIFT_CURIO_VISUAL\visual-console-p2\drift-curio\evidence\P5_QA01_V2_KONTEXT_D31_20260828-223558",
  [string]$ExpectedHead
)
...
try {
  ...
  Write-Host "==> Sync exact audited P5 Kontext D4 head" -ForegroundColor Cyan
  ...
  $safety = "safety/local-before-p5-kontext-d4-" + (Get-Date -Format "yyyyMMdd-HHmmss")
  ...
  if (-not (Test-Path -LiteralPath $PriorEvidenceDir -PathType Container)) {
    throw "PRIOR_D31_EVIDENCE_DIR_NOT_FOUND"
  }

  Write-Host "P5_QA01_V2_KONTEXT_D4_LOCAL_PREP=PASS" -ForegroundColor Green
  Write-Host ("HEAD=" + $head)
  Write-Host ("PRIOR_EVIDENCE_DIR=" + $PriorEvidenceDir)
  Write-Host "==> Launch environment realism + geometry-locked wet photometry + bounded contact repair" -ForegroundColor Cyan

  $python = "D:\AI\APPS\ComfyUI_windows_portable\python_embeded\python.exe"
  $script = Join-Path $RepoRoot "tools\p5_qa01_kontext_d4_eval.py"
  if (-not (Test-Path -LiteralPath $python -PathType Leaf)) { throw "COMFYUI_EMBEDDED_PYTHON_NOT_FOUND" }
  if (-not (Test-Path -LiteralPath $script -PathType Leaf)) { throw "KONTEXT_D4_SCRIPT_NOT_FOUND" }

  $toolsDir = Join-Path $RepoRoot "tools"
  $bootstrap = Join-Path $env:TEMP ("P5_QA01_D4_BOOTSTRAP_" + $ExpectedHead.Trim().Substring(0,12) + ".py")
  $launcher = @'
import runpy
import sys
...
sys.argv[0] = script
sys.stdout.write("P5_D4_PYTHON_BOOTSTRAP=TOOLS_DIR_INSERTED\n")
sys.stdout.flush()
...
'@
  ...
  if ($code -ne 0) { throw "P5_KONTEXT_D4_EVAL_FAILED: exit=$code" }
  ...
  Write-Host "P5_QA01_V2_KONTEXT_D4_LOCAL_GATE=PASS" -ForegroundColor Green
  exit 0
} catch {
  Write-Host "P5_QA01_V2_KONTEXT_D4_LOCAL_GATE=FAIL" -ForegroundColor Red
