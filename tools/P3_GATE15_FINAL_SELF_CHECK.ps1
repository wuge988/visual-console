...
function Read-JsonArrayFromUrl([string]$Uri) {
...
if ($before -ne $remote) {
    ...
    if ($LASTEXITCODE -ne 0) {
        Fail "Local HEAD is not an ancestor of remote candidate"
    }
...
$latestJobs = @{}
foreach ($line in ((Read-Utf8Text $jobsJournal) -split "`r?`n")) {
...
$archiveByAsset = @{}
foreach ($line in ((Read-Utf8Text $archiveJournal) -split "`r?`n")) {
    if ([string]::IsNullOrWhiteSpace($line)) {
        continue
    }
    try {
...
if ($archiveByAsset.Count -lt 1) {
    Fail "No archived SC01 masters were found for this SKU"
}
...
foreach ($assetId in $archiveByAsset.Keys) {
    ...
    $job = @($latestJobs.Values | Where-Object {
    ...
    $matchingHistory = @($manifest.archive_history | Where-Object {
        $_.gate -eq "15" -and
...
$legacy = @($manifest.archive_history | Where-Object {
...
foreach ($ownedProcessId in $processIds.Keys) {
    $proc = Get-CimInstance Win32_Process -Filter "ProcessId = $ownedProcessId"
    $cmd = [string]$proc.CommandLine
    if ($cmd -notmatch "VISUAL_CONSOLE") {
        Fail "Port is owned by a non-Visual-Console process: PID=$ownedProcessId CMD=$cmd"
    }
...
$archiveUri = "http://127.0.0.1:4179/api/archive$query"
$afterRestart = @(Read-JsonArrayFromUrl $archiveUri)
Write-Host "RESTART_API_ARCHIVES = $($afterRestart.Count)"
...
$historyBefore = @($manifestBefore.archive_history | Where-Object {
    $_.gate -eq "15" -and $_.asset_id -eq $retryAssetId
}).Count
...
$historyAfter = @($manifestAfter.archive_history | Where-Object {
    $_.gate -eq "15" -and $_.asset_id -eq $retryAssetId
}).Count
$archiveCountAfter = @(Read-JsonArrayFromUrl $archiveUri).Count
