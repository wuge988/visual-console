param(
  ...
  [string]$PriorEvidenceDir = "E:\AI_PROJECTS\DRIFT_CURIO_VISUAL\visual-console-p2\drift-curio\evidence\P5_QA01_V2_KONTEXT_D3_20260828-211413",
  [string]$ExpectedHead
)
...
try {
  ...
  Write-Host "==> Sync exact audited P5 Kontext D3.1 head" -ForegroundColor Cyan
  ...
  $safety = "safety/local-before-p5-kontext-d31-" + (Get-Date -Format "yyyyMMdd-HHmmss")
  ...
  if (-not (Test-Path -LiteralPath $PriorEvidenceDir -PathType Container)) {
    throw "PRIOR_D3_EVIDENCE_DIR_NOT_FOUND"
  }

  Write-Host "P5_QA01_V2_KONTEXT_D31_LOCAL_PREP=PASS" -ForegroundColor Green
  Write-Host ("HEAD=" + $head)
  Write-Host ("PRIOR_EVIDENCE_DIR=" + $PriorEvidenceDir)
  Write-Host "==> Re-run only Stage 2 with latent-preserving noise mask" -ForegroundColor Cyan

  $python = "D:\AI\APPS\ComfyUI_windows_portable\python_embeded\python.exe"
  $script = Join-Path $RepoRoot "tools\p5_qa01_kontext_d31_eval.py"
  if (-not (Test-Path -LiteralPath $python -PathType Leaf)) { throw "COMFYUI_EMBEDDED_PYTHON_NOT_FOUND" }
  if (-not (Test-Path -LiteralPath $script -PathType Leaf)) { throw "KONTEXT_D31_SCRIPT_NOT_FOUND" }

  $toolsDir = Join-Path $RepoRoot "tools"
  $bootstrap = Join-Path $env:TEMP ("P5_QA01_D31_BOOTSTRAP_" + $ExpectedHead.Trim().Substring(0,12) + ".py")
  $launcher = @'
import runpy
import sys
...
sys.argv[0] = script
sys.stdout.write("P5_D31_PYTHON_BOOTSTRAP=TOOLS_DIR_INSERTED\n")
sys.stdout.flush()
...
'@
  ...
  if ($code -ne 0) { throw "P5_KONTEXT_D31_EVAL_FAILED: exit=$code" }
  ...
  if ($null -ne $reviewLine) {
    ...
    if (Test-Path -LiteralPath $review -PathType Leaf) {
  ...
  Write-Host "P5_QA01_V2_KONTEXT_D31_LOCAL_GATE=PASS" -ForegroundColor Green
  exit 0
} catch {
  Write-Host "P5_QA01_V2_KONTEXT_D31_LOCAL_GATE=FAIL" -ForegroundColor Red
