    ...
    <main>
      ...
      <section>
        <div class="page-title">
          <div><em>生产工作台 · P1</em><h1>iPhone 直接采集到当前 SKU</h1><p>先完成局域网真机链路，再接 ComfyUI 批量生产。</p></div>
        </div>
        ...
        <div class="top-grid">
          <article class="card">
            ...
            <div class="hint">二维码会把 Site + 当前 SKU 一起绑定，手机端无需重新输入。</div>
          </article>

          <article class="card capture-card">
            <div class="capture-copy">
              <h3>手机采集</h3>
              <p>iPhone 16e 与电脑连接同一个 Wi‑Fi，然后扫描二维码。</p>
              <button class="primary" :disabled="creatingSession" @click="createMobileSession">{{ creatingSession ? "生成中…" : "生成手机上传二维码" }}</button>
            </div>
            <div class="qr-slot">
              <template v-if="mobileSession"><img :src="mobileSession.qr_data_url" alt="手机上传二维码" /><small>30 分钟有效</small></template>
        ...
        <article v-if="mobileSession" class="mobile-link">
          <b>手机上传地址</b><code>{{ mobileSession.mobile_url }}</code><span>扫码失败时可在 iPhone Safari 手动输入。</span>
        </article>
