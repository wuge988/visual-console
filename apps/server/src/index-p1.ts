...
function invalidateExistingItemSessions(siteId: string, itemId: string) {
  for (const [sessionId, session] of sessions.entries()) {
    if (session.siteId === siteId && session.itemId === itemId) {
...
async function resolveRawAsset(siteId: string, itemId: string, assetId: string) {
  ...
  const filename = (await readdir(dir)).find(
    (name) => rawAssetId(siteId, itemId, name) === assetId,
  );
...
app.post("/api/mobile/sessions", async (req, reply) => {
  try {
    ...
    invalidateExistingItemSessions(profile.site_id, itemId);
...
<title>手机采集｜Visual Console</title>
<style>
:root{color-scheme:dark;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI","Noto Sans SC",sans-serif}
*{box-sizing:border-box}body{margin:0;background:#0a0c0d;color:#f2f0e9}main{max-width:660px;margin:auto;padding:22px 16px 40px}
.kicker{font-size:11px;color:#c6a56b;letter-spacing:.08em}.sku{font-size:25px;margin:6px 0 3px}.muted{color:#98a19d;font-size:13px;line-height:1.6}
.bound{margin-top:12px;padding:11px 12px;border:1px solid #3b3529;border-radius:12px;background:#15130f}.bound b{display:block;font-size:13px;color:#e8d2a5}.bound span{display:block;margin-top:4px;font-size:12px;color:#b7afa0}
.card{margin-top:14px;border:1px solid #2b3133;background:#111415;border-radius:16px;padding:14px}label.action{display:block;border:1px solid #394144;background:#181c1e;border-radius:12px;padding:15px;margin:9px 0;text-align:center;font-size:16px;font-weight:650}input{display:none}
.expiry{display:flex;justify-content:space-between;gap:10px;margin:2px 0 12px;font-size:12px;color:#9cc2a6}.expiry.expired{color:#e28c8c}.status{font-size:13px;line-height:1.65;color:#9cc2a6;white-space:pre-wrap;margin-top:12px}.bar{height:6px;background:#252b2d;border-radius:99px;overflow:hidden;margin-top:8px}.bar i{display:block;height:100%;width:0;background:#c6a56b}.recent{display:grid;grid-template-columns:repeat(3,1fr);gap:7px;margin-top:10px}.shot{aspect-ratio:1/1;border:1px solid #2d3335;border-radius:9px;background:#0c0f10;display:grid;place-items:center;overflow:hidden;font-size:10px;color:#87908c}.shot img{width:100%;height:100%;object-fit:cover}
</style>
</head>
<body>
<main>
<div class="kicker">视觉生产控制台 · 手机采集</div>
<div class="sku">${s.itemId}</div>
<div class="muted">站点：${site.display_name} / ${site.display_name_zh}</div>
<div class="bound"><b>所有照片和视频都将保存到当前 SKU</b><span>${s.itemId}</span></div>
<div class="card">
<div id="expiry" class="expiry"><span>Session 有效期</span><b id="remaining">计算中…</b></div>
<label class="action">拍摄照片<input id="photo" type="file" accept="image/*" capture="environment"></label>
<label class="action">拍摄视频<input id="video" type="file" accept="video/*" capture="environment"></label>
<label class="action">从照片 / 文件中多选<input id="multi" type="file" accept="image/*,video/*" multiple></label>
<div id="status" class="status">等待上传</div><div class="bar"><i id="bar"></i></div><div id="recent" class="recent"></div>
</div>
</main>
<script>
const token=${JSON.stringify(token)};
const expiresAt=${s.expiresAt};
const directLimit=${DIRECT_UPLOAD_LIMIT};
const statusEl=document.getElementById("status"),bar=document.getElementById("bar"),recent=document.getElementById("recent"),remaining=document.getElementById("remaining"),expiry=document.getElementById("expiry");
function updateRemaining(){const ms=expiresAt-Date.now();if(ms<=0){remaining.textContent="已过期";expiry.classList.add("expired");for(const id of["photo","video","multi"])document.getElementById(id).disabled=true;return}const h=Math.floor(ms/3600000),m=Math.floor((ms%3600000)/60000);remaining.textContent=h+" 小时 "+m+" 分钟"}
updateRemaining();setInterval(updateRemaining,60000);
function setProgress(text,percent){statusEl.textContent=text;bar.style.width=Math.max(0,Math.min(100,percent||0))+"%"}
function addRecent(file){const box=document.createElement("div");box.className="shot";if(file.type.startsWith("image/")){const img=document.createElement("img");img.src=URL.createObjectURL(file);box.appendChild(img)}else box.textContent="VIDEO\\n"+file.name;recent.prepend(box);while(recent.children.length>6)recent.lastChild.remove()}
function uploadDirect(file){return new Promise((resolve,reject)=>{const xhr=new XMLHttpRequest();xhr.open("POST","/api/mobile/upload?token="+encodeURIComponent(token));xhr.upload.onprogress=e=>{if(e.lengthComputable)setProgress("上传中："+file.name,Math.round(e.loaded/e.total*100))};xhr.onerror=()=>reject(new Error("NETWORK_ERROR"));xhr.onload=()=>{let data={};try{data=JSON.parse(xhr.responseText)}catch{}if(xhr.status>=200&&xhr.status<300)resolve(data);else reject(new Error(data.error||("HTTP_"+xhr.status)))};const fd=new FormData();fd.append("file",file);xhr.send(fd)})}
async function retry(fn,times=3){let last;for(let i=0;i<times;i++){try{return await fn()}catch(e){last=e;await new Promise(r=>setTimeout(r,800*(i+1)))}}throw last}
async function uploadChunked(file){const init=await fetch("/api/mobile/uploads/init?token="+encodeURIComponent(token),{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({filename:file.name,mime:file.type||"application/octet-stream",size_bytes:file.size})});const meta=await init.json();if(!init.ok)throw new Error(meta.error||"INIT_FAILED");const total=meta.total_chunks;for(let i=0;i<total;i++){const start=i*meta.chunk_size,end=Math.min(file.size,start+meta.chunk_size),blob=file.slice(start,end);await retry(async()=>{const r=await fetch("/api/mobile/uploads/"+meta.upload_id+"/chunks/"+i+"?token="+encodeURIComponent(token),{method:"PUT",headers:{"content-type":"application/octet-stream"},body:blob});if(!r.ok){const j=await r.json().catch(()=>({}));throw new Error(j.error||"CHUNK_FAILED")}});setProgress("分块上传："+file.name+"\\n"+(i+1)+" / "+total,Math.round((i+1)/total*100))}const fin=await fetch("/api/mobile/uploads/"+meta.upload_id+"/finalize?token="+encodeURIComponent(token),{method:"POST"});const result=await fin.json();if(!fin.ok)throw new Error(result.error||"FINALIZE_FAILED");return result}
async function uploadFiles(files){for(const file of files){try{setProgress("准备上传："+file.name,0);const result=file.size>directLimit?await uploadChunked(file):await uploadDirect(file);addRecent(file);setProgress("已完成："+file.name+"\\n已保存："+result.filename,100)}catch(e){setProgress("失败："+file.name+"\\n"+(e.message||e),0);return}}}
for(const id of["photo","video","multi"]){document.getElementById(id).addEventListener("change",async e=>{await uploadFiles(e.target.files);e.target.value=""})}
</script>
</body>
</html>`;
  } catch {
...
app.put("/api/mobile/uploads/:uploadId/chunks/:index", async (req, reply) => {
  try {
    ...
    if (!Number.isInteger(n) || n < 0 || n >= upload.totalChunks) {
