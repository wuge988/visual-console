...
test("P5 keeps QA01 disabled while freezing identity-first Aquarium scene architecture", async () => {
  ...
  assert.match(installGate, /download_curl_attempt_/);
  ...
  assert.match(installGate, /aria2_skipped_for_curl_partial=true/);
  assert.match(installGate, /maxCurlAttempts = 12/);
  assert.match(installGate, /maxNoProgressAttempts = 3/);
  assert.match(installGate, /curl_attempt_\$\{curlAttempt\}_before_bytes=/);
  assert.match(installGate, /curl_attempt_\$\{curlAttempt\}_after_bytes=/);
  assert.match(installGate, /curl_attempt_\$\{curlAttempt\}_delta_bytes=/);
  assert.match(installGate, /curl_stopped_after_no_progress=true/);
  assert.match(installGate, /MODEL_DOWNLOAD_FAILED_ALL/);
  assert.match(installGate, /partial_before_bytes=/);
  assert.match(installGate, /partial_sha256=/);
